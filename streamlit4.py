...
# ---------------------------------------------------------------------------
VIDEO1_PATH = Path("presion de alta.mp4")  # Primer vídeo
VIDEO2_PATH = Path("potencia.mp4")  # Segundo vídeo
...
function updateProb(t) {{
  ...
  const prob  = values[idx];
  const label = prob < 15 ? "n/a"
                          : "Ensuciamiento del Condensador";

  document.getElementById('probBox').innerHTML =
      `Probabilidad de Fallo: ${{prob}} %<br>Etiqueta: ${{label}}`;
}}
